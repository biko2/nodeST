...
''' 
Sumalib es la bibloteca para el control del nodeST y que lo hacemos mediante de la Raspberry Pico
Mediante esta biblioteca podemos programar el NodeST usando los LEDs, el botín y altavoz
    
En python la forma de usar la bibloteca es: from (desde) import (utilidades)
'''
from sumalib import Sumalib

'''
Los leds son el elemento proncipal que se pueden controlar desde el nodeST
Usremos los leds para entender los rudimentos básicos de la programación
...
Si quires profundizar más en los concepts basicos de colores te recomiendo este link:
...
En la linea donde pone COLOR_ROJO = (255,0,0), se esta asignando a la variable COLOR_ROJO = tres valores numericos que simplificando
Son la cantidad de rojo, verde y azul que queremos que tenga nuestro led.

Te sorprendera que el valor de rojo que es 255, el verde es 0 y el azul es 0. 
...
Como los leds son aditivos, la suma de los colores será el color final. Si ponemos (255, 255, 255) tedremos el color blanco más brillante
...
'''
Uno de los elementos más importantes que usaremos es un bucle infinuto
'''
while(True):
    leds[0] = COLOR_ROJO
    leds[1] = COLOR_ROJO
